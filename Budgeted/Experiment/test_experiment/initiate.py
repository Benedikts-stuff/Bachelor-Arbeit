...
from Budgeted.Experiment.test_experiment.bandits.random import Random_Bandit
...
if __name__ == "__main__":
    ...
    reward_function = AddDataReward(**parameter_fb)
    cost_function = AddDataCost(**parameter_fb)
    filename = "experiment_logs.csv"
    context = FacebookData

    executor = Executor(non_linear, reward_function, cost_function, context,
                        n_features=5, n_runs=1, b=budget, filename=filename)
    executor.run_all()