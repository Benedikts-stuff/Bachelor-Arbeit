...
class EpsilonGreedyBandit:
    def __init__(self, context_dim, num_actions, epsilon=0.1):
        ...
        self.models = [SingleArmNetwork(context_dim) for _ in range(num_actions)]
...
# Simulation
def run_2(seed, contexts, true_rewards):
    np.random.seed(seed)
    num_samples = 10000
    context_dim = 5
    num_actions = 3
    ...
    num_rounds = 10000
    contexts_seen = [[] for _ in range(num_actions)]
    rewards_observed = [[] for _ in range(num_actions)]
